...
CLASS lcx_no_connection DEFINITION
 INHERITING FROM cx_static_check.
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS:
      BEGIN OF zcl_bm_learning_s4d401,
...
CLASS lcl_flight DEFINITION ABSTRACT.
  ...
  PUBLIC SECTION.
    ...
    INTERFACES lit_output.
    ALIASES get_output FOR lit_output~get_output.
    TYPES:
      tab TYPE STANDARD TABLE OF REF TO lcl_flight WITH DEFAULT KEY,
...
CLASS lcl_flight IMPLEMENTATION.
  ...
  METHOD class_constructor.
    ...
    div(
    tstmp_seconds_between( tstmp1 = dats_tims_to_tstmp( date = @today,
  ...
  METHOD get_flights_by_carrier.
    ...
    IF NOT line_exists( flights_buffer[ carrier_id = i_carrier_id ] ).
      ...
      FIELDS carrier_id, connection_id, flight_date,plane_type_id,seats_max,seats_occupied,
             ...
             currency_conversion(  source_currency = currency_code,
...
*           price,currency_code
       ...
       APPENDING TABLE @flights_buffer " make sure the result is added to the content of internal table flights_buffer instead of replacing
        .
  ...
  METHOD constructor.
    ...
    TRY.
        DATA(flight_row) = flights_buffer[ carrier_id    = i_carrier_id
      ...
      CATCH cx_sy_itab_line_not_found.
        ...
        currency_conversion( source_currency    = currency_code,
         ...
         WHERE carrier_id    = @i_carrier_id
          ...
          INTO CORRESPONDING FIELDS OF @flight_row.
    ENDTRY.

    IF flight_row IS NOT INITIAL.
      me->carrier_id    = i_carrier_id.
      me->connection_id = i_connection_id.
      me->flight_date   = i_flight_date.

      planetype = flight_row-plane_type_id.
      seats_max = flight_row-seats_max.
      seats_occ = flight_row-seats_occupied.
      seats_free = flight_row-seats_free."flight_row-seats_max - flight_row-seats_occupied.
  ...
  METHOD get_description.
    ...
    APPEND |{ 'Duration:'(011)       }{ connection_details-duration }| TO r_result.
...
CLASS lcl_carrier DEFINITION CREATE PRIVATE .
  ...
  PUBLIC SECTION.
    INTERFACES lit_output.
    ALIASES: get_output FOR lit_output~get_output,
             t_output FOR lit_output~t_output,
             tt_output FOR lit_output~tt_output.
    ...
    "! @raising cx_abap_auth_check_exception |Authorization failed
    "! @raising zcx_bm_failed |Instantiation failed - evaluate the exception text for details.
    ...
    "! Search for a <strong>passenger</strong> flight between two airports that
    "! <ul>
  ...
  METHOD get_instance.
    ...
    SELECT SINGLE
      ...
      FIELDS currency_code,
             concat_with_space( carrier_id, name,1 ) AS name
       WHERE carrier_id = @i_carrier_id
       INTO @DATA(details).

    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_bm_failed
        EXPORTING
          textid     = zcx_bm_failed=>carrier_not_exist
*         previous   =
          carrier_id = i_carrier_id.
...
*          value = CONV #( i_carrier_id ).
    ...
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_bm_failed
        EXPORTING
          textid     = zcx_bm_failed=>carrier_no_read_auth
*         previous   =
          carrier_id = i_carrier_id.
